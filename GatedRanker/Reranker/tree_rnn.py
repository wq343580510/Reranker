...
class TreeRNN(object):
    ...
    def create_pairwise_rank(self):
        gate_states_better = self.compute_tree_with_gate(self.x1_2, self.emb_x1, self.tree_3, self.tree_states_2)
        gate_states_worse = self.compute_tree_with_gate(self.x2_1,self.emb_x2, self.tree_4,self.tree_states_1)
        ...
        train_margin = theano.function(train_inputs_margin,[loss],updates=self.adagrad(loss))
    ...
    def train_pairwise(self,better_root,worse_root,pred = False):
        ...
        id1_2 = np.array([id_x.index(c) for c in id_x],dtype='int32')
        id2_1 = np.array([id_x2.index(c) for c in id_x2],dtype='int32')
        self._check_input(x, tree)
        self._check_input(x_2, tree_2)
        if pred:
